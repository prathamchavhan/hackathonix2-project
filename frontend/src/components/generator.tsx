...
export default function Generator() {
  ...
  return (
    <>
      {/* LEFT: Sticky Sidebar */}
      <Sidebar history={history} setTopic={setTopic} clearHistory={clearHistory} />

      {/* RIGHT: Main Generator Content */}
      <div
        className="ml-0 lg:ml-[260px] min-h-screen w-full lg:w-[calc(100%-260px)] flex flex-col items-center justify-center py-8 px-4 md:px-8 bg-[url('/3.webp')] bg-cover bg-center bg-no-repeat relative"
        ref={containerRef}
      >
        ...
        {/* Header */}
        <div className="mb-8 space-y-4 text-center mt-4 md:mt-8 w-full">
        ...
        <div className="w-full max-w-4xl">
          <div className="w-full bg-white rounded-[2rem] p-2 shadow-2xl flex flex-col relative transition-all duration-300">
            <textarea
              ...
              className="w-full min-h-[70px] px-6 py-5 text-slate-800 text-xl font-medium placeholder:text-gray-400 placeholder:font-normal bg-transparent border-none focus:outline-none focus:ring-0 resize-none rounded-t-3xl"
              disabled={loading}
              onKeyDown={(e) => {
            ...
            <div className="flex items-center justify-between px-4 pb-3 pt-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  className={`p-2 rounded-full transition-colors flex items-center justify-center ${isListening ? 'bg-red-100 text-red-500 hover:bg-red-200' : 'hover:bg-gray-100 text-gray-500'}`}
                ...
                <button
                  type="button"
                  className={`p-2 rounded-full transition-colors flex items-center justify-center hover:bg-gray-100 ${voiceEnabled ? 'text-indigo-500' : 'text-gray-400'}`}
                  onClick={() => {
                  ...
                  >
                    ...
                    <div className="flex gap-2 shrink-0 mt-2 md:mt-0">
                      <Button
                        ...
                        className={`transition-colors rounded-full ${selectedTitle === title
